...
public class course {
    ...
    public void insertUpdateDeleteCourse(char operation, Integer id, String label, Integer hours) {
        ...
        if (operation == 'd') { // d for delete
            ...
            int yesOrNo = JOptionPane.showConfirmDialog(null, "This Score Will Be Also Deleted", "Delete Score", JOptionPane.OK_CANCEL_OPTION, 0);
    ...
    public boolean isCourseExist(String courseName) {
        ...
        PreparedStatement ps;
        try {
            ps = con.prepareCall("SELECT * FROM `course` WHERE `label` = ?");
    ...
    public void fillCourseJtable(JTable table) {
        ...
        PreparedStatement ps;
        try {
            ps = con.prepareCall("SELECT * FROM `course`");
